...
function ClubBoard() {
    ...
    const fetchMoreItems = () => {
        if (startIndex >= dummyClubList.length) return; // 모든 데이터를 이미 불러왔다면 더 이상 진행하지 않음
        setLoading(true);
        // 예시: 임의의 데이터를 더 불러온다고 가정합니다.
        setTimeout(() => {
            const nextItems = dummyClubList.slice(startIndex, startIndex + 10);
